...
_MAC_SNAPSHOT = collections.namedtuple("MacClipboardSnapshot", "temporary manifest")
...
def _macos_press(shortcut, delay):
    ...
    # Showing the indicator can leave the previously active application's
    # window non-key without visibly changing focus. Restore that relationship
    # immediately before the paste event is posted.
...
def _type_through_program(program, command, hint, text, delay):
    """Type text through the desktop's existing keyboard-injection tool."""
    ...
    try:
        result = subprocess.run(
            [*command, text], capture_output=True, text=True, timeout=30
        )
    except (subprocess.SubprocessError, OSError) as exc:
        raise PasteError(t("Could not run {tool}: {error}",
                           tool=program, error=exc)) from exc
...
Desktop = collections.namedtuple(
    ...
    "clipboard packages read_command copy_command shortcuts keyboard ready press type",
)
...
WAYLAND = Desktop(
    ...
    **_program_keyboard(
    ...
    type=functools.partial(
        _type_through_program,
        "ydotool", ["ydotool", "type", "--key-delay", "0", "--"],
...
X11 = Desktop(
    ...
    **_program_keyboard("xdotool", _xdotool_command),
    type=functools.partial(
        _type_through_program,
        "xdotool", ["xdotool", "type", "--clearmodifiers", "--"], "",
    ),
)
...
def _macos_snapshot():
    ...
    # TemporaryDirectory also registers an interpreter-exit finalizer. A normal
    # crash therefore does not leave sensitive clipboard representations in
    # the temporary directory if the restore step was never reached.
    temporary = tempfile.TemporaryDirectory(prefix="dikte-clipboard-")
    ...
    try:
        ...
        return _MAC_SNAPSHOT(temporary, manifest)
...
def _macos_restore(snapshot):
    ...
    environment = dict(os.environ, DIKTE_PASTEBOARD_DIR=snapshot.temporary.name)
    try:
        subprocess.run(
...
def type_out(text, delay=0.12):
    """Type text into the focused window without borrowing the clipboard."""
    desktop().type(text, delay)
