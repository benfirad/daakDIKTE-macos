...
if ((MACOS)); then
  say  "Auto-paste presses Cmd+V through macOS Accessibility, which needs permission:"
  ...
  # Fedora can report the service healthy while its root-owned socket refuses
  # every user paste. The socket being writable is the actual readiness test.
